(defproject product-api "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "product-api.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.0"]]}
             :uberjar {:aot [product-api.server]}}
  :main ^{:skip-aot true} product-api.server)
